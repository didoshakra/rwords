// words/page.jsx
//Переміщення рядків/кнопкивід станів/
"use client"
...
export default function WordsPage() {
  ...
  const moveSelectedRow = (direction) => {
    ...
    if (direction === "up" && idx > 0) {
      newIdx = idx - 1
    } else if (direction === "down" && idx < total - 1) {
      newIdx = idx + 1
    }
      ...
      <div ref={tableContainerRef} className="max-h-[500px] overflow-auto border border-gray-300 rounded shadow-sm">
        <table className="w-full border-collapse">
          <thead className="bg-gray-100 sticky top-0 z-10">
            <tr>
              {/* <th
                className="p-2 border cursor-pointer text-center select-none"
                onClick={() => (selectedIds.length === words.length ? clearSelection() : selectAll())}
              >
                {selectedIds.length === words.length ? "☑" : "☐"}
              </th> */}
          ...
          <tbody>
            {words.map((w, index) => (
              ...
              >
                ...
                <td className="p-2 border">{w.pn}</td>
                ...
                {/* <td className="p-2 border">
                  {isOwnerOrAdmin(w) && (
                    <div className="flex gap-2">
                      <button onClick={() => moveUpInState(index)}>🔼</button>
                      <button onClick={() => moveDownInState(index)}>🔽</button>
                      <button onClick={() => openEditModal(w)} className="text-blue-600 hover:underline text-sm">
                        ✏️
                      </button>
                      <button onClick={() => handleDelete(w)} className="text-red-600 hover:underline text-sm">
                        🗑️
                      </button>
                    </div>
                  )}
