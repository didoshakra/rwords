...
export default function PicturesPage() {
  ...
  const handleSubmit = (e) => {
    ...
    if (!pictures_name.trim()) return setMessage("Заповніть назву картини")
    ...
    startTransition(async () => {
      try {
        if (modal?.type === "edit") {
          ...
          setMessage("Картину оновлено")
        } else {
          await createPicture(data, user?.id)
          setMessage("Картину створено")
        }
        closeModal()
        loadPictures()
        // Щоб не було memory leak, коли закриваєш модалку:
        if (previewUrl) {
      ...
      <Modal open={!!modal} onClose={closeModal}>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          ...
          {/* <div>
            ...
            <input type="file" name="file" accept="image/*" required className="border p-2 rounded w-full" />

            <p className="text-xs text-gray-500 mt-1">JPG / PNG → буде автоматично конвертовано в WebP</p>
          </div> */}
          <div>
            <label className="block font-medium mb-1">Файл зображення</label>
            <input
              ...
              onChange={(e) => {
                ...
                // автопідстановка title (без розширення)
                const nameWithoutExt = file.name.replace(/\.[^/.]+$/, "")
                setTitle(nameWithoutExt)
              }}
            />
            {/* 4️⃣ Preview зображення */}
            {previewUrl && (
              <div className="mt-3">
                <img src={previewUrl} alt="Preview" className="max-h-48 rounded border object-contain" />
              </div>
            )}
